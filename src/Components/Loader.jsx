...
const Loader = () => {
  ...
  const [color, setcolor] = useState("bg-zinc-300");
  ...
  useEffect(() => {
    ...
    setInterval(() => {
      a = a + 1;
      if (a < 100) {
        document.getElementById("timee").innerHTML = a + "%";
      } else {
        document.getElementById("timee").innerHTML = "100%";
      }
    }, 25);
  ...
  return (
    <div
      ...
      className={`z-[1000] transition-all ease-linear duration-[0.5s] ${color} absolute top-0 left-0 w-full h-screen`}
    >
      <motion.div
        style={{ scale: `${scale}`, transition: "all ease 0.5s" }}
        className="bg-zinc-900 w-full h-screen relative pt-[100px] rounded-[30px]"
      >
        <div className="nav rounded-t-[30px] absolute top-0 left-0 w-full h-[80px] px-[40px] max-[1280px]:px-[20px] flex justify-start items-center">
          <div className="logo">
            <svg
              width="72"
              height="30"
        ...
        <div>
          <h1
            style={{ opacity: `${opacity1}`, transition: "all ease 1s" }}
            id="timee"
            className="present text-white text-[10vw] absolute bottom-[40px] right-[40px]"
          >
            0%
          </h1>
        </div>
